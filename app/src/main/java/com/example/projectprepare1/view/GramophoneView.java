package com.example.projectprepare1.view;
...
public class GramophoneView extends View {
    ...
    public GramophoneView(Context context, AttributeSet attrs) {
        ...
        if(drawable == null){
            bitmap = BitmapFactory.decodeResource(getContext().getResources(), R.drawable.mark);
        } else{
        ...
        srcRect = new Rect();
        dstRect = new Rect();
        //Log.d("music"," " +dstRect.height()+"   init dest height ");
    ...
    private void setBitmapRect(Rect src, Rect dst){
        ...
        if(bitmapWidth <= diameter){
            src.left = 0;
            src.right = bitmapWidth;   //初始化src 矩形属性
        } else {
        ...
        dst.set(-pictureRadius, -pictureRadius, pictureRadius, pictureRadius);  //设置目标矩形属性，泳衣之后运算
    }
    ...
    private void drawDisk(Canvas canvas, float degree){
        ...
        //  Log.d("music"," " +dstRect.height()+"  dstRes  height ");
        canvas.drawBitmap(bitmap, srcRect, dstRect, discPaint);
        canvas.restore();
    }
    /**
     * 解决大图加载OOM问题
     * 这也是网上最普遍方式，也是最有通用的，采样率可以理解成：
     *          当采样率为４，表示将４个点“合并”为一个点来读出，
     *          缩小图片尺寸的同时也减少了图片占用空间，
     *          这样解码得到出来的图片占用空间自然比原图少。
    ...
    private Bitmap getMusicPicBitmap(int musicPicSize, int musicPicRes) {
        ...
        int sample = imageWidth / musicPicSize;
         Log.d("music",String.valueOf(sample));
        int dstSample = 1;
        if (sample > dstSample) {
            dstSample = sample;
        }
